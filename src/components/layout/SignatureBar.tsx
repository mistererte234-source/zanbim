...
export function SignatureBar() {
  ...
  return (
    <div className="w-full bg-zinc-950/90 border-b border-zinc-800/80 text-xs py-1.5 px-4 backdrop-blur-md relative z-40">
      <div className="max-w-7xl mx-auto flex flex-wrap items-center justify-between gap-2 text-zinc-300">
        ...
        <div className="flex items-center gap-2 font-medium">
          ...
          <span className="font-semibold text-white">{jawa.weton}</span>
          <span className="text-zinc-500">•</span>
          <span className="text-zinc-300 font-mono text-[11px]">{jawa.hijriStr}</span>
          <span className="text-zinc-500 hidden sm:inline">•</span>
          <span className="text-zinc-400 text-[11px] hidden sm:inline">({jawa.gregorianStr} M)</span>
        </div>

        {/* Jadwal Sholat Badge with Modal Toggle */}
        <div className="relative">
          <button
            onClick={() => setShowModal(!showModal)}
            className="flex items-center gap-2 px-2.5 py-1 rounded-lg bg-zinc-900/90 hover:bg-zinc-800 border border-zinc-800/80 transition-all font-mono text-[11px] text-zinc-200"
          >
            <Moon className="w-3.5 h-3.5 text-amber-400" />
            <span>Mendatang: <strong className="text-white">{sholat.nextPrayerName} ({sholat.nextPrayerTime})</strong></span>
          ...
          {showModal && (
            <div className="absolute right-0 top-8 z-50 w-64 glass-panel p-4 rounded-2xl border border-zinc-800 shadow-glow flex flex-col gap-3">
